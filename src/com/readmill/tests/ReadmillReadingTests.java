...
public class ReadmillReadingTests extends ReadmillTestCase {
	...
	// private String testResourcePath = "/com/readmill/tests/resources/";
	...
	@Test
	public void testInitFromJSON() throws JSONException, DatatypeConfigurationException {
		// Load the sample JSON string from
		// /com/readmill/tests/resources/sample_reading_response.json"
		...
		assertEquals("is private: ", false, reading.isPrivate());
		assertEquals(
				"The stories about the early days of Thefacebook was really great, hearing about how they were hacking like crazy from that house in Palo Alto. Although, like all tech-books it slowed down towards the end. The last 100 pages was a fight.",
				reading.getClosingRemark());
		...
		assertEquals("locations: ",
				"http://api.readmill.com/readings/21/locations",
				reading.getLocations());
		assertEquals("permalinkUrl; ",
				"http://readmill.com/henrik/reads/the-facebook-effect",
				reading.getPermalinkUrl());
		assertEquals("uri: ", "http://api.readmill.com/readings/21",
				reading.getUri());
		assertEquals("periods: ",
				"http://api.readmill.com/readings/21/periods", reading.getPeriods());
		assertEquals("highlights: ",
				"http://api.readmill.com/readings/21/highlights",
				reading.getHighlights());
		assertEquals("average period time: ", "5100.0",
				reading.getAveragePeriodTime());
		
		// book
		assertEquals("author of book: ", "David Kirkpatrick",
				reading.getBook().getAuthor());
		assertEquals("title of book: ", "The Facebook Effect",
				reading.getBook().getTitle());
		assertEquals("permalink of book: ", "the-facebook-effect",
				reading.getBook().getPermalink());

		// user
		assertEquals("id of user: ", 2, reading.getUser().getId());
		assertEquals("username: ", "henrik", reading.getUser().getUserName());
		assertEquals("fullname: ", "Henrik Berggren",
				reading.getUser().getFullName());
	}
	...
	public void testConvertToJSON() throws JSONException,
		...
		assertEquals("estimated time left: ", 0,
				json.optLong("estimated_time_left", 0));
		...
		assertEquals("createdAt: ", "2010-11-29T20:25:56Z",
				json.optString("created_at"));
		assertEquals("touchedAt; ", "2010-12-14T10:36:21Z",
				json.optString("touched_at"));
		assertEquals("startedAt: ", "2010-11-29T20:25:56Z",
				json.optString("started_at"));
		assertEquals("duration: ", "25500", json.optString("duration"));
		assertEquals("locations: ",
				"http://api.readmill.com/readings/21/locations",
				json.optString("locations"));
		assertEquals("permalinkUrl; ",
				"http://readmill.com/henrik/reads/the-facebook-effect",
		...
		assertEquals("uri: ", "http://api.readmill.com/readings/21",
				json.optString("uri"));
		assertEquals("periods: ",
				"http://api.readmill.com/readings/21/periods",
				json.optString("periods"));
		assertEquals("highlights: ",
				"http://api.readmill.com/readings/21/highlights",
				json.optString("highlights"));
		assertEquals("average period time: ", "5100.0",
				json.optString("average_period_time"));

		// book
		assertEquals("author of book: ", "David Kirkpatrick", json
				.optJSONObject("book").optString("author"));
		assertEquals("title of book: ", "The Facebook Effect", json
				.optJSONObject("book").optString("title"));
		assertEquals("permalink of book: ", "the-facebook-effect", json
				.optJSONObject("book").optString("permalink"));

		// user
		assertEquals("id of user: ", 2, json.optJSONObject("user").optInt("id"));
		assertEquals("username: ", "henrik", json.optJSONObject("user")
				.optString("username"));
		assertEquals("fullname: ", "Henrik Berggren", json
				.optJSONObject("user").optString("fullname"));
	}
}
