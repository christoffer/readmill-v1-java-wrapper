...
public class ApiWrapper implements ReadmillAPI, Serializable {
  ...
  public Token authorizationCode(String code) throws IOException {
    ...
    mToken = requestToken(Request.to(Endpoints.TOKEN).with("grant_type", AUTHORIZATION_CODE, "client_id", mClientId, "client_secret", mClientSecret, "redirect_uri", mRedirectUri, "code", code));
    return mToken;
  }
  ...
  public Token refreshToken() throws IOException {
    ...
    mToken = requestToken(Request.to(Endpoints.TOKEN).with("grant_type", REFRESH_TOKEN, "client_id", mClientId, "client_secret", mClientSecret, "refresh_token", mToken.refresh));
    return mToken;
  }
  ...
  public URI authorizationCodeUrl() {
    return getURI(Request.to(Endpoints.AUTHORIZE).with("redirect_uri", mRedirectUri, "client_id", mClientId, "response_type", "code", "mobile", "1"), false, false);
  }
  ...
  protected Token requestToken(Request request) throws IOException {
    ...
    HttpResponse response = getHttpClient().execute(env.webHost, request.buildRequest(HttpPost.class));
    ...
    if (status == HttpStatus.SC_OK) {
      ...
      throw status == HttpStatus.SC_UNAUTHORIZED ? new InvalidTokenException(status, sError) : new IOException(status + " " + response.getStatusLine().getReasonPhrase() + " (" + sError + ")");
    }
  }
      ...
      httpClient = new DefaultHttpClient(new ThreadSafeClientConnManager(params, registry), params) {
        {
          ...
          getCredentialsProvider().setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT, ReadmillAPI.REALM, OAUTH_SCHEME), OAuth2Scheme.EmptyCredentials.INSTANCE);

          getAuthSchemes().register(ReadmillAPI.OAUTH_SCHEME, new OAuth2Scheme.Factory(ApiWrapper.this));
        }

        @Override
        protected HttpContext createHttpContext() {
          HttpContext ctxt = super.createHttpContext();
          ctxt.setAttribute(ClientContext.AUTH_SCHEME_PREF, Arrays.asList(ReadmillAPI.OAUTH_SCHEME, "digest", "basic"));
          return ctxt;
        }
        ...
        protected RequestDirector createClientRequestDirector(HttpRequestExecutor requestExec, ClientConnectionManager conman, ConnectionReuseStrategy reustrat, ConnectionKeepAliveStrategy kastrat, HttpRoutePlanner rouplan, HttpProcessor httpProcessor, HttpRequestRetryHandler retryHandler, RedirectHandler redirectHandler, AuthenticationHandler targetAuthHandler, AuthenticationHandler proxyAuthHandler, UserTokenHandler stateHandler, HttpParams params) {
          return getRequestDirector(requestExec, conman, reustrat, kastrat, rouplan, httpProcessor, retryHandler, redirectHandler, targetAuthHandler, proxyAuthHandler, stateHandler, params);
        }
      };
    }
    return httpClient;
  ...
  public HttpResponse prepareAndExecute(Request request, Class<? extends HttpRequestBase> method) throws IOException {
    refreshTokenIfOld();
  ...
  public static Header createOAuthHeader(Token token) {
    return new BasicHeader(AUTH.WWW_AUTH_RESP, "OAuth " + (token == null || !token.valid() ? "invalidated" : token.access));
  }
  ...
  protected HttpRequest addHeaders(HttpRequest req) {
    System.out.println("ApiWrapper: addHeaders with token: " + mToken.access);
